...
export class ReportData {
  // Stock crítico - solo insumos activos
  ...
  // Valor del inventario - solo insumos activos
  async valorInventario() {
    const result = await pool.query(`
      SELECT 
        COALESCE(SUM(stock_actual * precio_costo_unitario), 0) AS valor_total_inventario
      FROM insumos
      WHERE activo = true
    `);
  ...
  async getBalanceRango(desde: string, hasta: string) {
    const sqlVentas = `
      SELECT 
        COUNT(id_pedido) as cant_ventas,
  ...
  async getHistorialCompras(desde: string, hasta: string) {
    ...
    SELECT 
      c.id_compra,
      DATE_TRUNC('day', c.fecha_compra)::date as fecha_compra,
